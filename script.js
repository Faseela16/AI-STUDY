function generatePlan() {
    ...
    const subjects = subjectsInput.split(",").map(s => s.trim().toLowerCase()); // Convert subjects to lowercase for consistency
    const weeks = 4;
    
    // Define different topics for each subject
    const topics = {
       "math": ["Algebra – Linear Equations", "Quadratic Equations", "Statistics – Probability", "Calculus – Differentiation"],
        "science": ["Physics – Motion and Forces", "Biology – Cell Structure", "Chemistry – Atomic Structure", "Earth Science – Plate Tectonics"],
        "history": ["Ancient Civilizations", "Renaissance and Reformation", "Age of Enlightenment", "World War I and II"],
        "java": ["Basic Syntax", "Control Flow Statements", "Object-Oriented Programming", "Collections Framework"],
        "python": ["Variables and Data Types", "Control Flow – If/Else", "Functions", "Data Structures – Lists and Dictionaries"],
        "c": ["Basic Syntax", "Functions", "Pointers", "Memory Management"],
        "c++": ["Object-Oriented Programming", "STL – Standard Template Library", "Inheritance", "Polymorphism"],
        "english": ["Grammar – Parts of Speech", "Writing – Essays and Reports", "Vocabulary – Word Usage", "Literature – Analyzing Texts"]
    };

    let studyTimePerWeek = (timeInput / (weeks * subjects.length)).toFixed(1); // Distribute time per subject and week
    let studyPlanHTML = "";
    ...
    for (let week = 1; week <= weeks; week++) {
        ...
        subjects.forEach(subject => {
            let topicList = topics[subject];  // Get the topic list for the subject
            
            if (!topicList) {
                topicList = ["General Study"]; // If subject is unknown, assign "General Study"
            }

            let topicIndex = (week - 1) % topicList.length; // Cycle through topics correctly

            studyPlanHTML += `
                <div class="study-topic">
                    <strong>${subject.charAt(0).toUpperCase() + subject.slice(1)}:</strong> ${topicList[topicIndex]}  
                    <br>⏳ Study Time: ${studyTimePerWeek} hours
                </div>
            `;
        });
...
const API_KEY = 'AIzaSyBih76lKef_HerTDsQ3_dVXKOBpc6CVGP8'; 
